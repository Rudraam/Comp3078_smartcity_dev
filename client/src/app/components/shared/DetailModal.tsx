...
import { X, MapPin, Clock, Calendar, Star, Users, Navigation, ExternalLink, DollarSign, Wifi, Dumbbell, UtensilsCrossed, Car, Waves, Sparkles, Coffee, Wind } from "lucide-react";
...
function ModalWrapper({ isOpen, onClose, children }: ModalWrapperProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className="relative bg-[#1a1d26] rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-10 bg-black/50 hover:bg-black/70 backdrop-blur-sm text-white w-9 h-9 rounded-full flex items-center justify-center transition-colors"
        >
          <X className="w-5 h-5" />
        </button>
        {children}
  ...
  return (
    <ModalWrapper isOpen={isOpen} onClose={onClose}>
      <div className="relative h-64">
        ...
        {restaurant.openNow && (
          <span className="absolute top-4 left-4 bg-green-500 text-white px-3 py-1 rounded-lg text-xs font-semibold">
            Open Now
          </span>
        )}
      </div>
      ...
      <div className="p-6 -mt-12 relative">
        ...
        <div className="flex flex-wrap items-center gap-4 text-sm mb-6">
          <div className="flex items-center gap-1.5 bg-[#23262f] px-3 py-1.5 rounded-lg">
            <Star className="w-4 h-4 text-yellow-400 fill-yellow-400" />
            <span className="font-semibold">{restaurant.rating}</span>
            <span className="text-[#99a1af]">({restaurant.reviews} reviews)</span>
          </div>
          <div className="flex items-center gap-1 bg-[#23262f] px-3 py-1.5 rounded-lg">
        ...
        <p className="text-[#99a1af] text-sm mb-6 leading-relaxed">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="bg-[#23262f] hover:bg-[#2a2e3a] transition-colors text-white px-5 py-3 rounded-lg font-medium flex items-center gap-2 text-sm"
          >
            View Menu
            <ExternalLink className="w-3 h-3" />
          </a>
      ...
      <div className="p-6 -mt-12 relative">
        ...
        <div className="flex flex-wrap items-center gap-4 text-sm mb-6">
          <div className="flex items-center gap-1.5 bg-[#23262f] px-3 py-1.5 rounded-lg">
            ...
            <span className="text-[#99a1af]">({hotel.reviews} reviews)</span>
          </div>
          <div className="bg-[#23262f] px-3 py-1.5 rounded-lg">
            <span className="text-xl font-bold">${hotel.pricePerNight}</span>
            <span className="text-[#99a1af] text-xs ml-1">/night</span>
          </div>
        </div>
        ...
        <div className="mb-6">
          <h3 className="text-sm font-semibold text-[#99a1af] uppercase tracking-wider mb-3">Amenities</h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {hotel.amenities.map((amenity) => {
              const Icon = amenityIcons[amenity] || Sparkles;
              return (
                <div key={amenity} className="flex items-center gap-2 bg-[#23262f] px-3 py-2.5 rounded-lg">
      ...
      <div className="p-6 -mt-12 relative">
        ...
        <div className="flex flex-wrap items-center gap-4 text-sm mb-6">
          ...
          <div className="bg-[#23262f] px-3 py-1.5 rounded-lg">
            ...
            {typeof event.price === "number" && <span className="text-[#99a1af] text-xs ml-1">/ticket</span>}
          </div>
        </div>
